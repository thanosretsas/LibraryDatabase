...
LibraryDatabase/SQL/create_database.sql
@thanosretsas
thanosretsas Add files via upload
Latest commit a0962a2 Jun 4, 2023
History
1 contributor
199 lines (180 sloc) 6.14 KB
...
create definer = root@localhost trigger if not exists increase_copies_available
    after delete
    on reservation
BEGIN
    IF OLD.status = 'active' THEN
        ...
        WHERE id = OLD.book_school_id;
    END IF;
END;

create definer = root@localhost trigger if not exists reduce_copies_available
    after insert
    on reservation
BEGIN
    IF NEW.status = 'active' THEN
        UPDATE book_school
        SET copies_available = copies_available - 1
        WHERE id = NEW.book_school_id;
    END IF;
END;
create definer = root@localhost trigger if not exists update_copies_on_loan
    after insert
    on loan
BEGIN
    UPDATE book_school
...
LibraryDatabase/create_database.sql at master · thanosretsas/LibraryDatabase · GitHub
